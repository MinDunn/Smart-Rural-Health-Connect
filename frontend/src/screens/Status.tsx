...
const StatusScreen = ({ profile, setProfile, bmi, bmiStatus, patientId }: StatusProps) => {
  const handleUpdateMetrics = async () => {
    if (!patientId) return;
    try {
      const { iotApi } = await import('../lib/api');
      const metrics = [
        { type: 'blood_pressure', value: parseFloat(profile.bp.split('/')[0]) || 0, unit: 'mmHg' },
        { type: 'blood_sugar', value: parseFloat(profile.sugar) || 0, unit: 'mmol/L' },
        { type: 'heart_rate', value: parseFloat(profile.hr) || 0, unit: 'bpm' },
        { type: 'spo2', value: parseFloat(profile.spo2) || 0, unit: '%' }
      ];
      ...
      for (const m of metrics) {
        await iotApi.saveMetric({
          ...
          value: m.value,
          unit: m.unit,
          isStable: true, // Placeholder logic
          status: 'stable'
        });
      }
      alert('Đã cập nhật chỉ số sức khỏe lên hệ thống!');
    } catch (error) {
