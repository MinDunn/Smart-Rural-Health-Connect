...
export class PatientsService {
  ...
  async findByUserId(userId: string): Promise<Patient> {
    ...
    if (!patient) {
      ...
      console.log(`[PATIENTS] Creating lazy patient record for user: ${userId}`);
  ...
  async updatePatientInfo(userId: string, data: any): Promise<Patient> {
    ...
    if (patient.user && patient.user.profile) {
      if (data.firstName !== undefined) patient.user.profile.firstName = data.firstName;
      if (data.lastName !== undefined) patient.user.profile.lastName = data.lastName;
      if (data.phone !== undefined) patient.user.profile.phone = data.phone;
      if (data.address !== undefined) patient.user.profile.address = data.address;
    ...
    if (data.emergencyContacts !== undefined) patient.emergencyContacts = data.emergencyContacts;
